...
App này có 3 chế độ chạy, tự nhận diện trong [UiUtils.html](UiUtils.html):
...
Mở [FirebaseConfig.html](FirebaseConfig.html), điền `FIREBASE_CONFIG` (apiKey, projectId, …) lấy ở bước 1.
> Các khoá web này **công khai được** (Firestore bảo vệ bằng Security Rules, không phải bằng việc giấu key).

Kiểm tra `CAPACITY` ở [ClientConfig.html](ClientConfig.html) đúng sĩ số mong muốn, và **danh sách `ICONS`** có đủ số đội bạn cần.
...
> ⚠️ Trong file rules có `function cap() { return 10; }` — **số này phải khớp `CAPACITY`** ở [ClientConfig.html](ClientConfig.html). Đổi sĩ số thì sửa cả hai.

## 4. Build trang tĩnh & đẩy lên GitHub Pages

```powershell
.\build.ps1          # gộp các include thành docs\index.html (chế độ firebase không cần -ScriptUrl)
git add -A
...
- **Ghi (tham gia đội): atomic bằng `runTransaction`** ([Api.html](Api.html)) — mili-giây, không phải khóa tuần tự 1–2s như Apps Script. Không ai thành người thứ 11; không trùng email; 1 người 1 đội.
...
- Thêm đội = thêm phần tử vào mảng `ICONS` ([ClientConfig.html](ClientConfig.html)); **mỗi `icon` phải là một emoji DUY NHẤT** (nó là khoá định danh đội). Cần nhiều đội thì dùng các emoji khác nhau.
...
- `emails/{key}` cho `get` (transaction cần) nên người biết chính xác 1 email có thể dò "email này đã đăng ký chưa" (không đọc được PII). Cấm liệt kê cả bảng (`list:false`) nên không enumerate được.
- File [Config.gs](Config.gs)/[Game.gs](Game.gs)/[Handlers.gs](Handlers.gs) (Apps Script) giờ là **legacy** — chế độ `firebase` không dùng tới. Có thể giữ làm dự phòng hoặc xoá.

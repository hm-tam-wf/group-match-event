...
Mở [firestore.rules](firestore.rules), trong `match /events/{eventId}/signups` thay placeholder bằng UID thật:
...
1. Mở [docs/js/config.js](docs/js/config.js), đổi `EVENT_ID` sang nhãn mới (chỉ **chữ thường, số, gạch ngang**), vd `"su-kien-2026-q2"`.
2. (Tuỳ chọn) chỉnh `FIELDS` / `ICONS` / `CAPACITY` cho sự kiện đó. Nhớ `CAPACITY` khớp `cap()` trong [firestore.rules](firestore.rules).
